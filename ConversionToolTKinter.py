...
def convert(sv,v,ot): #Conversion Variables
	#Conversion IF statements to each conversion
	if (sv.get().isdigit()): 
		print("v.get() :"+str(v.get())+" | sv.get(): "+str(sv.get())) #Print of Answer
		if v.get() == 0:
			ot.set("Please select an option for conversions") #Instructions
		elif v.get() == 1:
...
app.mainloop()  #Loop until application is closed
